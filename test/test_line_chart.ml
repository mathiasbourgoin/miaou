...
let test_empty_chart () =
  let chart =
    Line_chart.create ~width:10 ~height:5 ~series:[] ()
  in
  let output = Line_chart.render chart ~show_axes:false ~show_grid:false ~thresholds:[] () in
...
let test_single_point () =
  ...
  let has_marker =
    try ignore (Str.search_forward (Str.regexp "●") output 0); true
...
let test_two_points_line () =
  ...
  in
  let chart = Line_chart.create ~width:10 ~height:5 ~series:[series] () in
  let output = Line_chart.render chart ~show_axes:true ~show_grid:false ~thresholds:[] () in
...
let test_grid_rendering () =
  ...
  let output = Line_chart.render chart ~show_axes:true ~show_grid:true ~thresholds:[] () in
...
let test_update_series () =
  ...
  let chart' = Line_chart.update_series chart ~label:"test" ~points:new_points in
  let output = Line_chart.render chart' ~show_axes:false ~show_grid:false ~thresholds:[] () in
...
let test_add_point () =
  ...
  let chart = Line_chart.create ~width:10 ~height:5 ~series:[series] () in
  let chart' =
    Line_chart.add_point chart ~label:"test" ~point:{Line_chart.x = 5.0; y = 5.0; color = None}
  in
  let output = Line_chart.render chart' ~show_axes:false ~show_grid:false ~thresholds:[] () in
...
let test_with_title () =
  ...
  in
  let chart =
    Line_chart.create ~width:10 ~height:5 ~series:[series] ~title:"Test Chart" ()
  in
  let output = Line_chart.render chart ~show_axes:false ~show_grid:false ~thresholds:[] () in
