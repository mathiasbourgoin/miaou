...
let render_sparkline_sdl sparkline ~color ~thresholds =
  ...
  | None ->
      ...
      Sparkline.render sparkline ~focus:false ~show_value:false ~color ~thresholds ()
  | Some ctx ->
      ...
      let label_and_value_width = 17 in (* "CPU:  99.9 " or "NET:  99.9 KB/s " *)
      ...
      Sparkline_sdl.render_sdl info sparkline ~focus:false ~show_value:false 
        ~color ~thresholds () ;
...
let render_line_chart_sdl chart ~thresholds =
  ...
  | Some ctx ->
      ...
      Line_chart_sdl.render_sdl info chart ~show_axes:true ~show_grid:false 
        ~thresholds () ;
