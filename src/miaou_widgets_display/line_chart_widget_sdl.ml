...
let render_series_sdl info series x_min x_max y_min y_max ?thresholds () =
  ...
  let map_x x =
    if x_range = 0. then pixel_width / 2
      ...
      int_of_float
        (((x -. x_min) /. x_range) *. float_of_int pixel_width)
  in
  ...
  let map_y y =
    if y_range = 0. then pixel_height / 2
      ...
      pixel_height
      - int_of_float
          (((y -. y_min) /. y_range) *. float_of_int pixel_height)
  in
  ...
  List.iteri
    (fun i (point : Line_chart_widget.point) ->
      ...
      | Some next_point ->
          ...
          List.iter
            (fun (x1, y1) ->
              ignore (render_draw_point renderer x1 y1))
            fill_points ;
...
let render_axes_sdl info =
  ...
  (* Y axis (left) *)
  ignore (render_draw_line renderer info.x info.y info.x (info.y + pixel_height))

(* Main SDL render function *)
let render_sdl info chart ~show_axes ~show_grid ?thresholds () =
  let x_min, x_max, y_min, y_max = calculate_bounds (Line_chart_widget.get_series chart) in
