  ...
  if Sparkline_widget.is_empty sparkline then ()
    ...
    List.iteri
      (fun i value ->
        ...
        let rect = Rect.create ~x ~y ~w:(max 2 (int_of_float x_step)) ~h:height in
...
let render_with_label sparkline ~label ~focus ?color ?(thresholds = []) () =
  Sparkline_widget.render_with_label sparkline ~label ~focus ?color ~thresholds ()
